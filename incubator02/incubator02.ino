...
Metro metrodaychange=Metro(17280000);
...
{ 
  ...
  delay(1000);
  Serial.begin(9600);
  numberofdays=EEPROM.read(1)+1;//since starting day is 0
  onoff= EEPROM.read(0);
  onoff=onoff+1;
  if(onoff>100)
  {
...
{ 
if(px>3)// to get all sensor reading
{ 
if(abs(tem1-tem2)>1)
{
tscount=tscount+1; 
...
if(tscount==8)
  {
       tscount=0;
       correction=t1-t2;
       EEPROMWritelong(2,(correction*100));
  }
}
}
...
if(metrodaychange.check()==1)
{
  ...
  EEPROM.update(1,numberofdays);  
}
if (metrotoday.check() == 1) 
{
dd=numberofdays;//dayOfMonth1;  
if(dd<23)
{
  set_temp=37.70;  
}
