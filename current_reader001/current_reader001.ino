...
void loop() {
  ...
  voltage =  voltage_raw - QOV + 0.027 ;// 0.000 is a value to make voltage zero when there is no current
  ...
  if(abs(current) > cutOffLimit ){
    ...
    Serial.print("V, I: ");
    Serial.print(current*230,2); // print the current with 2 decimal places
    Serial.println("A");
  }else{
